...
function category(v){const s=compact(v);return s==='헬스▶1개월'?'health':s==='골프▶스크린타석▶1개월'?'screen':s==='골프▶기본▶1개월'?'golf':s==='헬스▶세대당5회무료▶1개월'?'free':'';}
...
function analyze(rows,opt={}){
 ...
 const maps={health:new Map(),screen:new Map(),golf:new Map(),free:new Map()};const unknown=new Map();const missing=[];
 ...
 const groups={};for(const c of ['health','screen','golf'])groups[c]=[...maps[c].values()].map(h=>({...h,members:[...h.members.values()],count:h.members.size})).sort(sort);
 groups.free=[...maps.free.values()].map(h=>{const paid=maps.health.get(h.key);const members=new Map();for(const [id,m] of h.members)members.set(id,{...m,role:'5회 무료'});for(const [id,m] of paid?.members||[])members.set(id,{...m,role:members.has(id)?'월권 + 5회 무료':'헬스 월권'});return {...h,members:[...members.values()],count:members.size,freeCount:h.members.size,paidCount:paid?.members.size||0,freeDuplicate:h.members.size>=2,mixed:!!paid,kind:[h.members.size>=2?'무료권 다인원':'',paid?'월권·무료권 병행':''].filter(Boolean).join(' / ')};}).sort(sort);
...
const api={parseGrid,analyze,category,names};if(typeof module!=='undefined')module.exports=api;root.HouseholdCore=api;
